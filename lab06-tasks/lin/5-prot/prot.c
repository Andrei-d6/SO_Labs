...
static void segv_handler(int signum, siginfo_t *info, void *context)
{
	...
	if (how[page] == PROT_NONE  || how[page] == PROT_WRITE) {

		how[page] = PROT_READ;
		rc = mprotect(p + page*pageSize, pageSize, PROT_READ);
		DIE(rc == -1, "PROT_READ");

		printf("increase protection with PROT_READ to page %d\n", page);
	} else {
		...
		printf("increase protection with PROT_WRITE to page %d\n", page);
	}
...
int main(void)
{
	...
	//ch = p[2*pageSize]; 
	//printf("This should need PROT_READ\n");
	printf("%c\n", p[2*pageSize]);
	p[2*pageSize] = 'c';
